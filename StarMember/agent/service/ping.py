#
#   Fork from https://github.com/kyan001/ping3
# 
#   modified for gevent concurrency.
#
...
def receive_one_ping(my_socket, ID, dest_addr, timeout):
    """
    ...
    while True:
        ...
        if type != 8 and packetID == ID and addr == dest_addr:
...
def ping(dest_addr, timeout=4, unit="s", src_addr=None, ttl=64):
    ...
    delay = receive_one_ping(my_socket, my_ID, dest_addr, timeout)  # in seconds
    my_socket.close()
    if delay is None:
        return None
    if unit == "ms":
